...
def validate_seed(seed):
    ...
    return dict(state='PASS',seed=seed,transferred_initial_models=models,
...
def main():
    checks=[validate_seed(seed) for seed in SEEDS]
    write(HERE/'preflight_results.json',dict(state='PASS',seeds=SEEDS,per_seed=checks,smoke_physical_steps=8000*len(SEEDS)))
    print('Three-seed transfer PASS',SEEDS,flush=True)
