...
def modify_args(args):
    ...
    if 'lsvpe' in args.shutter and args.interp not in ['scatter', 'none', None]:
    ...
    log_group_dir = getattr(args, 'log_group_dir', '')

    if args.test:
        exp_name = f'{args.log_root}/test'
        args.steps_til_summary = 10
    else:
        if log_group_dir not in ['', None]:
            dir_name = str(log_group_dir)
        else:
