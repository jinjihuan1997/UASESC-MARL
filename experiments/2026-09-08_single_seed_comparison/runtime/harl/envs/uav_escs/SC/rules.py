...
def mode_choice(env, observation, kind):
    ...
    for j in valid:
        ...
        scores[j] = (env.omega_Q*q_gain/env.n_ds
                     - env.omega_Lambda*count*loads[j]/env.Lambda_ref/env.n_uav
...
def rule_actions(env, obs, available, kind, rng):
    ...
    if kind == "Random":
        sut = rng.uniform(-1, 1, size=env.n_uav).astype(np.float32)
        ...
        if np.ptp(pressure) > 1e-12:
            sut = (2*(pressure-pressure.min())/np.ptp(pressure)-1).astype(np.float32)
    actions = [sut]
    for n in range(env.n_uav):
        if kind == "Random":
