    ...
    try:
        ...
        while update < trainer.total_updates and not stopped:
            ...
            values = dict(update=update, steps=update*trainer.batch, mean_training_reward=mean_reward,
                ...
                actor_learning_rates=[a.actor_optimizer.param_groups[0]['lr'] for a in trainer.actors],
                ...
                warmup_equal_draw_fraction=float(trainer.warmup_equal_draws/trainer.batch) if trainer.fixed_resources else None,
                resource_share_mean=(trainer.resource_share_sum/trainer.batch).cpu().tolist(),
                unused_budget_mean=(trainer.unused_budget_sum/trainer.batch).cpu().tolist(),
                deliveries_mean=(trainer.delivery_sum/trainer.batch).cpu().tolist(),
                mode_budget_counts=trainer.mode_budget_counts.reshape(3,trainer.env.U,6,17).cpu().tolist(),
        ...
        expected=[i for i in range(len(final)) if i>0 or config['training_design']['arm']!='mode_only']
        if not all(final[i] != trainer.initial_actors[i] for i in expected):
            raise AssertionError('An actor did not update')
        if config['training_design']['arm']=='mode_only':
            assert final[0]==trainer.initial_actors[0], 'Fixed resource SUT must remain untrained'
