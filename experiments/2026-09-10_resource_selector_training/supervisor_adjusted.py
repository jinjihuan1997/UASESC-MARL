...
def verify_amendment():
    ...
    assert amendment['original_manifest_sha256']==digest(HERE/'manifest.json')
    assert amendment['supervisor_sha256']==digest(__file__)
    return amendment
...
def run(resume):
    ...
    # Preserve the first thermal event's minimum cool-down across scheduler restart.
    import datetime
    resume_at=datetime.datetime.fromisoformat(amendment['not_before_utc']).timestamp()
    thermal.cooling=True;thermal.paused_since=time.monotonic()-max(0,180-(resume_at-time.time()))
    try:
        ...
        while pending or active:
            ...
            if not thermal.cooling:
                # Prioritize all queued training cores before assigning evaluation work.
                cores=sorted(m['resources']['cores'],key=lambda c:not any(j['kind']=='training' and j['core']==c for j in pending))
                for core in cores:
                    if len(active)>=amendment['maximum_simultaneous_workers']:break
            ...
            write(HERE/'status.json',dict(state='cooling' if thermal.cooling else 'training_and_evaluating',updated_utc=stamp(),supervisor_pid=os.getpid(),
                ...
                cpu_max_c=cpu,gpu_max_c=gpu,estimated_training_seconds_remaining=eta,eta_excludes_queueing=True,maximum_simultaneous_workers=amendment['maximum_simultaneous_workers'],failed_jobs=failures))
...
def start(resume):
    ...
    with (HERE/'.launcher.lock').open('a+') as lock:
        ...
        with (HERE/'logs/supervisor.log').open('a') as log:
            p=subprocess.Popen([sys.executable,'-u',str(HERE/'supervisor_adjusted.py'),'run']+(['--resume'] if resume else []),cwd=HERE,
