...
ROOT=Path(__file__).resolve().parent/'targeted_followup'
VARIANTS=['quality_rule_resources_only','alias_probability_aggregation']
...
def actions(env,actors,obs,masks,variant):
    ...
    if variant=='quality_rule_resources_only' and gid==2:
        resource=rule_actions(env,'m5_equal')[0]
    else:
        resource=actors[0].act(obs[:,0],rnn,active,masks[:,0],deterministic=True)[0]
    post,_,available=env.allocate_resources(resource)
    modes=[]
    for i in range(1,env.n_agents):
        action=actors[i].act(post[:,i],rnn,active,available[:,i],deterministic=True)[0]
        if variant=='alias_probability_aggregation':
            groups=[]
            for m in range(env.M):
                group=next((x for x in groups if torch.equal(env.profiles[:,m],env.profiles[:,x[0]])),None)
                if group is None: groups.append([m])
                else: group.append(m)
            assert groups==[[0,4,8,12],[1],[2],[3],[5,9,13],[6],[7],[10],[11],[14],[15]],groups
            for group in groups:
                assert all(torch.equal(available[:,i,group[0]],available[:,i,m]) for m in group)
            _,_,distribution=actors[i].evaluate_actions(post[:,i],rnn,action,active,available[:,i],active)
            probs=distribution.components[0]['dist'].probs.squeeze(1)
            group_probs=torch.stack([probs[:,group].sum(-1) for group in groups],-1)
            np.testing.assert_allclose(arr(group_probs.sum(-1)),1,atol=1e-6,rtol=0)
            representatives=torch.tensor([g[0] for g in groups])
            choice=representatives[group_probs.argmax(-1)]
            action=torch.nn.functional.one_hot(choice,env.M).to(action.dtype)
        modes.append(action)
...
def main():
    ...
    write(ROOT/'protocol.json',dict(created_utc=stamp(),purpose='targeted_resource_and_alias_followup',
        ...
        limitation='Post-hoc development diagnostics. Quality-only equal allocation is a hybrid, not a pure RL improvement. Alias aggregation alters deterministic inference only; no retraining or independent holdout.'))
    (ROOT/'logs').mkdir()
    def launch(index):
    ...
    lines+=['','quality_rule_resources_only：仅质量指令时资源均分，模式仍用RL；其他指令全部保留RL。它是定位用混合方法。',
        'alias_probability_aggregation：同一物理模式的编号概率相加，再选概率最大的物理模式；不改任何模型参数或指令路由。',
        '',f'独立审计{episodes}回合、{episodes*600}时隙。原始输入和权重未变；只有推理控制组合变化。']
