...
from harl.envs.uav_escs.reward_objective import reward_components as compute_reward_components
...
class SCUAVEnv:
    ...
    Physical UAVs 1..N select a common semantic mode. Cached streams are
    scheduled deterministically. Only non-fixed roles have learning agents.
    """
    ...
    def __init__(self, args):
        ...
        self.fixed_mode_rule = bool(self._arg("fixed_mode_rule", False))
        self.fixed_resources = bool(self._arg("fixed_resources", False))
        self.active_agent_ids = ([] if self.fixed_resources else [0]) + (
            [] if self.fixed_mode_rule else list(range(1, 1 + self.n_uav)))
        if not self.active_agent_ids:
            raise ValueError("No trainable role; use a rule baseline with the full interface")
        self.n_agents = len(self.active_agent_ids)
        self.scheduler = str(self._arg("scheduler", "aoi"))
        if self.scheduler not in ("aoi", "round_robin"):
            raise ValueError("scheduler must be aoi or round_robin")
        self.rr_cursor = np.zeros(self.n_uav, dtype=int)
        self.explicit_instruction_schedule = self._arg("explicit_instruction_schedule", None)
        ...
        if self._arg("semantic_mode_selection", "all_modes") != "all_modes":
            raise ValueError("This table contains SCI/rate rows, not training-SNR groups")
        self.semantic_mode_selection = "all_modes"
        self.n_mu_modes = self.n_semantic_modes
        ...
        self.reward_load_ref = self._finite_float("reward_load_ref", 60000.0, lower=1e-9)
        self.reward_load_scale = self._finite_float("reward_load_scale", 0.2, lower=0.0)
        if "use_task_auxiliary_reward" in args:
            raise ValueError("This version always trains the full objective; remove the obsolete auxiliary switch")
        self.A_max = self._finite_float("A_max", 600.0, lower=1.0)
        self.aoi_reward_ref = self._finite_float("aoi_reward_ref", 10.0, lower=1e-9)
        self.backhaul_availability = self._finite_float("backhaul_availability", 0.6, lower=1e-9)
        if self.backhaul_availability > 1:
            raise ValueError("backhaul_availability must lie in (0,1]")
        self.reward_weights_by_instruction = np.asarray(self._arg(
            "reward_weights_by_instruction", [[.5,.4,.1],[.2,.7,.1],[.7,.2,.1]]), dtype=float)
        if (self.reward_weights_by_instruction.shape != (self.num_instructions,3)
            or not np.isfinite(self.reward_weights_by_instruction).all()
            or (self.reward_weights_by_instruction < 0).any()
            or not np.allclose(self.reward_weights_by_instruction.sum(axis=1),1)):
            raise ValueError("Reward weights must be finite nonnegative [quality,AoI,load] rows summing to one")
        ...
        self.last_common_evaluation_reward = 0.0
        ...
        self.last_uav_raw_actions = np.zeros((self.n_uav, self.n_mu_modes), dtype=float)
        # SUT simplex allocation; UAV categorical mode. No scheduling head.
        self.sut_act_slices = {"beta_logits": slice(0, self.n_uav)}
        self.uav_act_slices = {"mu_logits": slice(0, self.n_mu_modes),
                               "mode_logits": slice(0, self.n_mu_modes)}
        ...
        if self.use_instruction_constraints:
            ...
            if self.include_instruction_constraints_in_obs:
                self.sut_instruction_obs_dim += 1
                self.uav_instruction_obs_dim += 4 + 2
                self.share_instruction_obs_dim += 1 + self.n_uav
        ...
        self.action_space = [self.action_space[i] for i in self.active_agent_ids]
    ...
    def _build_semantic_mode_lookup(self):
        # SNR indexes table columns. Every row remains available to every bucket.
        if self.n_semantic_modes != 16 or not self.semantic_library.profile:
            raise ValueError("The frozen 16-row average table is required")
    ...
    def _get_instruction_id_for_slot(self, slot_t: int) -> int:
        ...
        if self.explicit_instruction_schedule is not None:
            return next(int(g) for t, g in reversed(self.explicit_instruction_schedule) if t <= slot_t)
    ...
    def _reset_instruction_schedule(self) -> None:
        if self.explicit_instruction_schedule is not None:
            schedule = self.explicit_instruction_schedule
            if (not schedule or schedule[0][0] != 0 or
                any(len(pair) != 2 or int(pair[0]) != pair[0] or int(pair[1]) != pair[1]
                    or not 0 <= pair[0] < self.max_steps or not 0 <= pair[1] < self.num_instructions
                    for pair in schedule) or
                any(schedule[i][0] >= schedule[i+1][0] for i in range(len(schedule)-1))):
                raise ValueError("Schedule must have increasing integer slots, begin at 0, and use valid IDs")
            self.current_instruction_id = int(schedule[0][1])
    ...
    def _instruction_context_for_buckets(self, bucket_ids: np.ndarray | None = None) -> Tuple[int, str, float, np.ndarray]:
        ...
        return gid, name, a_limit, q_req

    def _refresh_instruction_context_from_current(self) -> None:
        gid, name, a_limit, q_req = self._instruction_context_for_buckets()
    ...
    def _derived_schedule_scores(self, n, mode_id, q_t, A_t, q_req_gb):
        candidate = self.candidate_mask(n, mode_id, q_t, q_req_gb)
        scores = np.full(self.n_ds, -np.inf, dtype=float)
        if self.scheduler == "aoi":
            scores[candidate] = A_t[candidate]
        else:
            ds = self.ds_by_uav[n]
            scores[ds] = -((np.arange(ds.size) - self.rr_cursor[n]) % ds.size)
            scores[~candidate] = -np.inf
        return scores, candidate


    def _project_beta(self, raw: np.ndarray) -> np.ndarray:
        # Actor encodes Dirichlet coordinates as raw=2*p-1. Recover those
        # coordinates directly instead of applying a second softmax.
        beta = np.maximum((np.asarray(raw, dtype=float) + 1.0) / 2.0, 0.0)
        if not np.isfinite(beta).all():
            raise ValueError("Non-finite resource action")
        total = float(beta.sum())
        beta = beta / total if total > 0 else np.full(self.n_uav, 1.0/self.n_uav)
    ...
    def _normalize_actions_by_agent(self, actions):
        from harl.envs.uav_escs.SC.rules import fixed_mode_actions
        seq = list(actions)
        if len(seq) != self.n_agents:
            raise ValueError(f"Expected {self.n_agents} active actions, got {len(seq)}")
        physical = [np.zeros(self.n_uav)] + [np.zeros(self.n_mu_modes) for _ in range(self.n_uav)]
        if self.fixed_mode_rule:
            physical[1:] = fixed_mode_actions(self)
        for action, physical_id, space in zip(seq, self.active_agent_ids, self.action_space):
            row = np.asarray(action, dtype=float).reshape(-1)
            if row.size != space.shape[0] or not np.all(np.isfinite(row)):
                raise ValueError(f"Invalid action for physical agent {physical_id}: {row.shape}")
            physical[physical_id] = row
    ...
    def _update_backhaul_budget(self):
        ...
        self.Phi_bh = self.delta_T * self.backhaul_availability * self.B_bh
    ...
    def _build_obs_multi(self) -> np.ndarray:
        ...
        return np.asarray(obs, dtype=np.float32)[self.active_agent_ids]
    ...
    def _sut_instruction_obs(self) -> np.ndarray:
        ...
        gid, _, a_limit, _ = self._instruction_context_for_buckets()
        parts = []
        if self.include_instruction_id_in_obs:
            parts.append(self._instruction_one_hot(gid))
        if self.include_instruction_constraints_in_obs:
            parts.append(np.asarray([a_limit / self.A_limit_context_ref], dtype=float))
    ...
    def _uav_instruction_obs(self, n: int) -> np.ndarray:
        if not self.use_instruction_constraints:
            return np.zeros(0, dtype=float)
        bucket_id = self._effective_snr_bucket_for_uav(n)
        gid, _, a_limit, q_req = self._instruction_context_for_buckets(
    ...
    def _share_instruction_obs(self) -> np.ndarray:
        ...
        gid, _, a_limit, q_req = self._instruction_context_for_buckets(buckets)
        parts = []
        if self.include_instruction_id_in_obs:
            parts.append(self._instruction_one_hot(gid))
        if self.include_instruction_constraints_in_obs:
    ...
    def _build_share_obs_multi(self) -> np.ndarray:
        ...
        if state.size != self.share_obs_dim:
            raise AssertionError(f"Shared observation size {state.size} != {self.share_obs_dim}")
        return np.tile(state[None, :], (self.n_agents, 1)).astype(np.float32)

    def _build_action_available_masks(self) -> np.ndarray:
        masks = np.ones((1 + self.n_uav, self.common_act_dim), dtype=np.float32)
        ...
        for n in range(self.n_uav):
            ...
            masks[1 + n] = row
        return masks[self.active_agent_ids]
    ...
    def step(self, actions):
        ...
        for n in range(self.n_uav):
            ...
            mu_order = list(np.lexsort((np.arange(self.n_mu_modes), -mu_logits)))
            ...
            for mu_id in mu_order:
                ...
                m = self._global_mode_id(bucket_id, mu_id)
                score_by_ds, candidate = self._derived_schedule_scores(n, m, q_t, A_t, Q_req_gb[n])
                if not np.any(candidate):
                    continue
                bucket_has_feasible = True
                budget_left = float(self.Phi_bh[n])
                ordered_ds = np.where(candidate)[0]
                ordered_ds = ordered_ds[np.lexsort((ordered_ds, -score_by_ds[ordered_ds]))]
                selected = []
                for k in ordered_ds:
            ...
            if self.scheduler == "round_robin":
                last_local = int(np.flatnonzero(self.ds_by_uav[n] == chosen_streams[-1])[0])
                self.rr_cursor[n] = (last_local + 1) % self.ds_per_uav
        ...
        A_norm = A_next / max(float(self.aoi_reward_ref), 1.0e-9)
        ...
        A_tail = (
            float(
                np.mean(
                    np.maximum(
                        (A_next - self.aoi_tail_threshold) / max(float(self.aoi_reward_ref), 1.0e-9),
                        0.0,
                    )
                )
            )
        ...
        terms = compute_reward_components(self, quality_gain, float((chi*self.Lambda_sem).sum()), A_t, A_next, gid)
        quality_term, load_term, aoi_term = (float(terms[k]) for k in ('quality_term','load_term','aoi_term'))
        A_mean, A_fair, A_tail = (float(terms[k]) for k in ('age_mean_term','age_max_term','age_tail_term'))
        aoi_state_term = aoi_term
        quality_contrib = float(terms['quality_credit'])
        load_penalty = -float(terms['resource_cost'])
        aoi_penalty = -sum(float(terms[k]) for k in ('age_mean_cost','age_max_cost','age_tail_cost'))
        base_reward = float(terms['base_reward'])
        quality_violation_bucketwise = float(np.mean(quality_violations)) if quality_violations else 0.0
        aoi_violation_max = float(terms['max_aoi_violation'])
        recv_aoi_bonus = float(terms['reception_credit'])
        common_evaluation_reward = reward = float(terms['objective_reward'])
        reward_components = {
            'reward_quality': quality_contrib, 'reward_load_cost': load_penalty,
            'reward_aoi_penalty': aoi_penalty,
            'penalty_aoi_violation': -float(terms['service_violation_cost']),
            'bonus_aoi_reduction': recv_aoi_bonus,
        }
        ...
        self.last_common_evaluation_reward = float(common_evaluation_reward)
    ...
    def _build_info(self, slot_t: int, reward: float) -> Dict[str, Any]:
        ...
        info = {
            ...
            "is_instruction_switch_step": bool(slot_t in [t for t, _ in self.explicit_instruction_schedule[1:]])
                if self.explicit_instruction_schedule is not None
                else bool(self.instruction_mode_strategy in ("switch_once", "random_switch_once")
                          and slot_t == self.instruction_switch_step),
            ...
            "omega_Q": float(self.reward_weights_by_instruction[gid_slot,0]),
            "omega_Lambda": float(self.reward_weights_by_instruction[gid_slot,2]),
            "omega_A": float(self.reward_weights_by_instruction[gid_slot,1]),
            ...
            **(self.semantic_library.diagnostic() if self.semantic_library.enabled else {}),
            ...
            "common_evaluation_reward": float(self.last_common_evaluation_reward),
            "active_agent_ids": list(self.active_agent_ids),
            "scheduler": self.scheduler,
            "explicit_instruction_schedule": self.explicit_instruction_schedule,
