...
class SemanticModeLibrary:
    ...
    def from_config(
        ...
        *,
        ...
        if not enabled:
            return cls("disabled", [], profile_path=None)
        ...
        sets = data.get("sets", {})
        if selected == ALL_FIXED_SNR_MU_GRID_SET:
            modes: List[Dict[str, Any]] = []
            for source_set in FIXED_SNR_MU_SWEEP_SETS:
                if source_set not in sets:
                    raise ValueError(f"Missing semantic model subset {source_set!r} for {selected!r}")
                for item in sets[source_set].get("modes", []):
                    mode = dict(item)
                    mode["mode_id"] = len(modes)
                    mode["source_model_set"] = source_set
                    modes.append(mode)
            return cls(selected, modes, profile_path=profile_path)
    ...
    def diagnostic(self) -> Dict[str, Any]:
        return {
            ...
            "semantic_mode_trained_snr_db": self.trained_snr_db.copy(),
            "semantic_mode_mu_comm": self.mu_comm.copy(),
            "semantic_mode_rate_level_proxy": self.rate_level_proxy.copy(),
