"""Detached, checkpoint-aware six-job queue with bounded thermal pauses."""
import argparse
import fcntl
import signal
import subprocess
...
from helpers import *
from three_seed_train import ThermalControl,process_info,recorded_process_alive
from benchmark import gpu_temperature
...
def training_progress(m):
    ...
    for item in m['jobs']:
        ...
        rows[item['id']]=dict(state=r.get('state','queued'),steps=steps,target=10_000_000,device=item['device'],core=item['core'],recoverable_update=r.get('recoverable_update',0))
        if r.get('last_timing') and steps<10_000_000:remaining.append((10_000_000-steps)/4000*r['last_timing']['total_seconds'])
    return rows,max(remaining) if remaining else None


def run_phase(m,phase,stopping,resume):
    items=m['jobs'] if phase=='training' else [dict(id=j['id'],output='evaluation/'+j['id']) for j in m['jobs']]+[dict(id='rules/'+r,output='evaluation/rules/'+r) for r in m['rules']]
    ...
    for item in items:
        ...
        if status.get('state')=='complete':
            if phase=='training':
                assert status['completed_steps']==10_000_000
        ...
        while pending or active:
            ...
            for slot in range(6):
                ...
                if phase=='training':
                    ...
                    item=pending.popleft();cmd=[sys.executable,'-u',str(HERE/'long_eval.py'),'--item',item['id']]
            ...
            write(HERE/'status.json',dict(state='cooling' if thermal.cooling else phase,updated_utc=stamp(),supervisor_pid=os.getpid(),
                completed_training_steps=sum(r['steps'] for r in rows.values()),total_training_steps=60_000_000,
                completed_training_jobs=sum(r['state']=='complete' for r in rows.values()),total_training_jobs=6,
...
def run(resume):
    ...
    try:
        ...
        verify();audit=read(HERE/'report/audit.json');assert audit['state']=='PASS' and audit['episodes']==2340
        write(HERE/'status.json',dict(state='complete',updated_utc=stamp(),completed_training_steps=60_000_000,completed_training_jobs=6,
            evaluation_episodes=2340,report=str(HERE/'REPORT.md'),science_status='results_ready_for_interpretation'))
        return 0
