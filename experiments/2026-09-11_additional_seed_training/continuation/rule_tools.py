...
def predict_reward(env,actions):
    ...
    gain=(((quality-p.Q_min_eval)/(p.Q_max-p.Q_min_eval)).clamp_min(0)*count).sum(-1)
    return reward_components(p,gain,(load*count).sum(-1),env.aoi.reshape(env.count,-1),next_aoi.reshape(env.count,-1),gid)['objective_reward']



def myopic_actions(env,equal=False):
    candidates=[rule_actions(env,name) for name in RULES if not equal or name.endswith('_equal')]
