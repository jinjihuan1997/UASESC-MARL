...
def main(run):
    ...
    assert json.loads((run/'status.json').read_text())['state'] in ('analyzing','complete')
        ...
        for episode in s['episodes']:
            ...
            with path.open() as f:
                for row in csv.DictReader(f):
                    for key in row:
                        if key not in ('fractions','modes','proposed_modes'):row[key]=float(row[key])
                    rows.append(row)
        data[method]=rows
    ...
    def metrics(rows):
        ...
        return dict(slots=len(rows),mean_reward=statistics.mean(r['common_reward'] for r in rows),
            ...
            proposal_override_fraction=sum(r['proposal_overridden_uavs'] for r in rows)/max(sum(r['delivering_uavs'] for r in rows),1),
            proposed_quality_infeasible_fraction=sum(r['proposed_quality_infeasible_uavs'] for r in rows)/max(sum(r['pending_uavs'] for r in rows),1))
    ...
    lines=['严格即时指令接口：单种子诊断结果','',f"种子85，两方法各{manifest['steps']:,}步；65组配对episode，5个外部种子，每episode600槽。",'',
    ...
    lines+=['','指令0/1/2分别为balance/aoi/quality。PSNR按交付数加权；其他均值按相应槽加权。','',
        ...
        '', '提议被执行器替换比例（分母为有交付的UAV槽）：'+json.dumps({m:result[m]['overall']['proposal_override_fraction'] for m in methods}),
        '提议质量不合格比例（分母为有待发更新的UAV槽）：'+json.dumps({m:result[m]['overall']['proposed_quality_infeasible_fraction'] for m in methods}),
