...
class OffPolicyBaseRunner:
    ...
    def run(self):
        ...
        update_num = int(  # SC number per train
    ...
    def sample_actions(self, available_actions=None):
        ...
        for agent_id in range(self.num_agents):
            action = []
            is_discrete = self.action_spaces[agent_id].__class__.__name__ == "Discrete"
            for thread in range(self.algo_args["train"]["n_rollout_threads"]):
                if (available_actions[thread] is None) or (not is_discrete):
