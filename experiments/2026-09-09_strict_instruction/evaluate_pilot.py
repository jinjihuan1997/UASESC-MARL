    ...
    for scenario,schedule in manifest['scenarios'].items():
        ...
        for seed in manifest['evaluation_seeds']:
            ...
            for t in range(600):
                ...
                proposed=np.array([int(np.argmax(actions[n+1])) for n in range(env.n_uav)])
                pending=np.array([bool(env.q_cache[n,env.ds_by_uav[n]].any()) for n in range(env.n_uav)])
                bad_quality=np.array([env.Q_hat_rec[n,int(env.ds_by_uav[n][0]),proposed[n]] <
                    env._q_req_for_instruction_bucket(env.current_instruction_id,env._effective_snr_bucket_for_uav(n))-1e-9
                ...
                row['modes']=json.dumps(env.last_selected_modes.tolist())
                row['proposed_modes']=json.dumps(proposed.tolist())
                delivering=env.last_selected_modes>=0
                row['delivering_uavs']=int(delivering.sum())
                row['pending_uavs']=int(pending.sum())
                row['proposal_overridden_uavs']=int((delivering & (env.last_selected_modes!=proposed)).sum())
                row['proposed_quality_infeasible_uavs']=int((pending & bad_quality).sum())
                rows.append(row)
