        ...
        for spec, head in zip(self.continuous_specs, self.continuous_heads):
            ...
            if kind == "simplex":
                ...
                logp_block = self._spread_event_logp(
                    # Evaluate the float32 action representation actually stored
                    # in the rollout, exactly as evaluate_actions decodes it.
                    # Near -1, the affine encoding loses precision in tiny shares.
                    dist.log_probs(0.5 * (raw_group + 1.0)),
                    cont_mask,
                    int(spec["dim"]),
                    x.dtype,
                    x.device,
