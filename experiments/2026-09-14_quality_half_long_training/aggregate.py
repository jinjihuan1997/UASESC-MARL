...
def main():
 ...
 ref=arrays(SHORT/'report/episode_scores.npz');assert ref['parents'].tolist()==m['seeds'] and ref['seeds'].tolist()==m['validation'] and ref['scenarios'].tolist()==scenes
 ...
 shortaudit=read(SHORT/'report/audit.json');assert shortaudit['state']=='PASS'
 for pi,seed in enumerate(m['seeds']):
  for si,scene in enumerate(scenes):
   key=f'seed_{seed}/steps_1000000/{scene}';path=SHORT/'evaluation'/f'{key}.npz';assert sha(path)==shortaudit['trace_sha256'][key]
   oldz=arrays(path);assert oldz['seeds'].tolist()==m['validation'];idx=ref['methods'].tolist().index('retrained_rl_Qhalf');np.testing.assert_array_equal(oldz['trace'][:,:,0].mean(0)*100,ref['scores_x100'][idx,pi,si])
 scores={name:ref['scores_x100'][i] for i,name in enumerate(ref['methods'].tolist())};scores['long_rl_Qhalf']=np.zeros((3,13,20))
 physical={};checks={};tables={};process={};episodes={};hashes={}
 for pi,seed in enumerate(m['seeds']):
  for step in [2000000,4000000,6000000,8000000,10000000]:
   for scene in (scenes if step==10000000 else ['fixed_0','fixed_1','fixed_2']):
    ...
    if step==10000000:
     scores['long_rl_Qhalf'][pi,scenes.index(scene)]=value
 ...
 summaries={name:STATS.summary(v,m,boot,ix,name in ['original_rl','C1','C3','retrained_rl_Qhalf','long_rl_Qhalf']) for name,v in scores.items()}
 paired={f'long_rl_Qhalf_minus_{name}':STATS.summary(scores['long_rl_Qhalf']-v,m,boot,ix,True) for name,v in scores.items() if name!='long_rl_Qhalf'}
 ...
 np.testing.assert_allclose(phys['groups']['overall_13']['score_x100'],summaries['long_rl_Qhalf']['conditional_mean']['overall_13']['mean'],atol=1e-12,rtol=0)
 refphys=read(PREVIOUS/'report/physical_statistics.json')['summary'];refphys['retrained_rl_Qhalf']=read(SHORT/'report/physical_statistics.json')['summary'];gaps={}
 for name in scores:
  if name=='long_rl_Qhalf':continue
  gaps[name]={}
  for group in ix:
   ...
   expected=paired[f'long_rl_Qhalf_minus_{name}']['conditional_mean'][group]['mean'];np.testing.assert_allclose(delta,expected,atol=1e-10,rtol=0);gaps[name][group]=dict(score_difference=expected,reward_component_differences=parts)
 train={};actualseeds=set();training_audits={}
 for seed in m['seeds']:
  folder=HERE/f'jobs/seed_{seed}';status=read(folder/'status.json');assert status['state']=='complete' and status['completed_steps']==10000000
  rows=[json.loads(x) for x in (folder/'training_metrics.jsonl').read_text().splitlines()];assert len(rows)==2250 and [r['steps'] for r in rows]==list(range(1004000,10000001,4000))
  assert all(r['trained_actor_ids']==[0,1,2,3] and r['stage']=='joint' for r in rows)
  counts=np.array([r['instruction_counts'] for r in rows]).sum(0);assert counts.sum()==9000000
  state,entry=checkpoint.load_checkpoint(folder/'checkpoints');assert state['update']==2500 and int(state['environment']['audited_physical_steps'])==10000000
  ...
  for opt in state['actor_optimizers']+[state['critic_optimizer']]:
   steps=sorted(set(int(v['step']) for v in opt['state'].values()));assert steps==[25000];step_counts.append(steps[0]-2500)
  selected=sorted((folder/'trajectory_audits').glob('episode_*.npz'));assert len(selected)==60
  ...
  train[str(seed)]=dict(environment_steps=9000000,cumulative_environment_steps=10000000,prior_environment_steps=1000000,actor_optimizer_steps=sum(step_counts[:4]),critic_optimizer_steps=step_counts[4],instruction_counts=counts.tolist(),newly_completed_episodes=15000,of_which_started_in_short_run=10,fully_new_complete_episodes=14990,partial_episodes=10,partial_episode_steps_each=400,online_audited_new_physical_steps=9000000,offline_complete_trajectory_audited_steps=360000,initial_actor_hashes=state['initial_actor_hashes'],final_actor_hashes=status['final_actor_hashes'],checkpoint_sha256=entry['sha256'])
  del state
 costs=[]
 ...
 write(HERE/'report/audit.json',dict(state='PASS',new_rl_environment_steps=27000000,cumulative_rl_environment_steps=30000000,new_actor_optimizer_steps=270000,new_critic_optimizer_steps=67500,new_evaluation_steps=900000,new_evaluation_complete_episodes=1500,preflight=read(HERE/'preflight.json'),formal_training=train,evaluation_checks=checks,training_offline_checks=training_audits,trace_sha256=hashes,reference_trace_sha256=oldaudit['trace_sha256'],reference_reused_episodes=6760,reference_new_computation_steps=0,protected_files=len(m['protected_sha256']),protected_files_unchanged=True,actual_environment_seeds=sorted(actualseeds),reserved_final_test_used=False,reserved_final_test_intersection=[]))
