...
def prepare(run):
    verify_reference()
    gates=['validation_cpu.json','validation_gpu.json','interface_checks.json','physics_preservation.json','resume_checks.json','resume_checks_gpu.json']
    ...
    write(run/'manifest.json',dict(purpose='strict_instruction_interface_single_seed_pilot',seed=85,steps=1_000_000,
        ...
        gates={name:sha(ROOT/name) for name in gates},protocol_sha256=sha(ROOT/'PROTOCOL.md'),
        parent_evidence=read(ROOT/'parent_evidence.json'),input_hashes=hashes,created_utc=stamp()))
...
def run_jobs(run):
    ...
    try:
        ...
        write(run/'status.json',dict(state='analyzing',utc=stamp()))
        with (run/'logs/analysis.log').open('a') as log:
            subprocess.run([sys.executable,str(run/'source/summarize_pilot.py'),'--run',str(run)],
