...
class OnPolicyBaseRunner:
    ...
    @staticmethod
    def _as_2d_action_batch(arr):
        """Normalize model action output to shape (n_threads, action_dim)."""
    ...
    def _pack_env_actions(self, per_agent_actions):
        """Pack per-agent action batches into env step input format."""
        if len(per_agent_actions) == 0:
            return np.zeros((0, 0, 0), dtype=np.float32)

        n_threads = int(per_agent_actions[0].shape[0])
        dims = []
        for a in per_agent_actions:
            if a.shape[0] != n_threads:
                raise ValueError(
                    "Inconsistent n_threads across agents in action collection: "
                    f"expected {n_threads}, got {a.shape[0]}."
                )
            dims.append(int(a.shape[1]))

        # Homogeneous action dims keep dense ndarray for compatibility/perf.
        if len(set(dims)) == 1:
            return np.stack(per_agent_actions, axis=1)

        # Heterogeneous action dims: use thread-major nested list.
        return [
            [per_agent_actions[agent_id][t].copy() for agent_id in range(self.num_agents)]
            for t in range(n_threads)
        ]

    @staticmethod
    def _select_agent_batch(data_by_agent, agent_id):
        """Select agent batch from either ndarray(thread,agent,dim) or list[agent]."""
        if isinstance(data_by_agent, (list, tuple)):
            return np.asarray(data_by_agent[agent_id], dtype=np.float32)
        return data_by_agent[:, agent_id]

    def _get_action_spec(self, agent_id):
        try:
            return getattr(self.envs.action_space[agent_id], "hybrid_action_spec", None)
        except Exception:
            return None

    def _rollout_available_action_batches(self, available_actions):
        ...
        for agent_id in range(self.num_agents):
            batches.append(
                np.asarray(
                    self._select_agent_batch(available_actions, agent_id),
                    dtype=np.float32,
                ).copy()
            )
        return batches

    def _sample_actor_step(
        self,
        agent_id,
        obs_batch,
        rnn_state_batch,
        ...
        if with_log_probs:
            ...
            return (
                self._as_2d_action_batch(_t2n(action)),
                self._as_2d_action_batch(_t2n(action_log_prob)),
                _t2n(rnn_state),
            )
        ...
        return self._as_2d_action_batch(_t2n(action)), None, _t2n(rnn_state)

    def _rollout_policy_actions(
        self,
        obs_batches,
        rnn_state_batches,
        mask_batches,
        ...
        action_collector = [None for _ in range(self.num_agents)]
        action_log_prob_collector = [None for _ in range(self.num_agents)]
        rnn_state_collector = [None for _ in range(self.num_agents)]

        current_avail = [
            None if batch is None else np.asarray(batch, dtype=np.float32).copy()
            for batch in available_batches
        ]

        for agent_id in range(self.num_agents):
            action_batch, log_prob_batch, rnn_state_batch = self._sample_actor_step(
                agent_id,
                obs_batches[agent_id],
                rnn_state_batches[agent_id],
                mask_batches[agent_id],
                current_avail[agent_id],
                deterministic,
                with_log_probs,
            )
            action_collector[agent_id] = action_batch
            action_log_prob_collector[agent_id] = log_prob_batch
            rnn_state_collector[agent_id] = rnn_state_batch

        return action_collector, action_log_prob_collector, rnn_state_collector, current_avail
    ...
    def collect(self, step):
        ...
        Returns:
            values, env_actions, actions, action_log_probs, rnn_states, rnn_states_critic
        """
        # collect actions, action_log_probs, rnn_states from n actors
        obs_batches = [self.actor_buffer[agent_id].obs[step] for agent_id in range(self.num_agents)]
        rnn_state_batches = [
            self.actor_buffer[agent_id].rnn_states[step] for agent_id in range(self.num_agents)
        ]
        mask_batches = [self.actor_buffer[agent_id].masks[step] for agent_id in range(self.num_agents)]
        available_batches = [
            self.actor_buffer[agent_id].available_actions[step].copy()
            if self.actor_buffer[agent_id].available_actions is not None
            else None
            for agent_id in range(self.num_agents)
        ]
        (
            ...
            rnn_state_collector,
            used_available_batches,
        ) = self._rollout_policy_actions(
            obs_batches,
            rnn_state_batches,
            mask_batches,
            available_batches,
        ...
        for agent_id in range(self.num_agents):
            if (
                self.actor_buffer[agent_id].available_actions is not None
                and used_available_batches[agent_id] is not None
            ):
                self.actor_buffer[agent_id].available_actions[step] = (
                    used_available_batches[agent_id].copy()
                )
        env_actions = self._pack_env_actions(action_collector)
        actions = action_collector
        action_log_probs = action_log_prob_collector
    ...
    def insert(self, data):
        ...
        (
            ...
            actions,  # ndarray(thread, agent, dim) or list[agent](thread, dim)
            action_log_probs,  # ndarray(thread, agent, dim) or list[agent](thread, dim)
        ...
        for agent_id in range(self.num_agents):
            self.actor_buffer[agent_id].insert(
                obs[:, agent_id],
                rnn_states[:, agent_id],
                self._select_agent_batch(actions, agent_id),
                self._select_agent_batch(action_log_probs, agent_id),
                masks[:, agent_id],
    ...
    def eval(self):
        ...
        while True:
            eval_obs_batches = [
                self._select_agent_batch(eval_obs, agent_id)
                for agent_id in range(self.num_agents)
            ]
            eval_rnn_batches = [
                eval_rnn_states[:, agent_id] for agent_id in range(self.num_agents)
            ]
            eval_mask_batches = [
                eval_masks[:, agent_id] for agent_id in range(self.num_agents)
            ]
            eval_available_batches = self._rollout_available_action_batches(
            ...
            (
                ...
                eval_rnn_batches,
                eval_mask_batches,
                eval_available_batches,
                deterministic=True,
                with_log_probs=False,
            )
            eval_rnn_states = np.stack(eval_rnn_collector, axis=1)

            eval_env_actions = self._pack_env_actions(eval_actions_collector)
            ...
            (
                ...
                eval_infos,
                eval_available_actions,
            ) = self.eval_envs.step(eval_env_actions)
            eval_data = (
                eval_obs,
                eval_share_obs,
                eval_rewards,
            ...
            for _ in range(self.algo_args["render"]["render_episodes"]):
                ...
                while True:
                    ...
                    (
                        ...
                        _,
                        eval_available_actions,
                    ) = self.envs.step(eval_env_actions[0])
            ...
            for _ in range(self.algo_args["render"]["render_episodes"]):
                ...
                while True:
                    ...
                    (
                        ...
                        _,
                        eval_available_actions,
                    ) = self.envs.step(eval_env_actions)
