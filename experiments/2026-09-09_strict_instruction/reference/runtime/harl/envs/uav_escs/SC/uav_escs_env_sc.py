...
class SCUAVEnv:
    ...
    def __init__(self, args):
        ...
        self.use_task_auxiliary_reward = bool(self._arg("use_task_auxiliary_reward", True))
    ...
    def _build_sut_obs(self) -> np.ndarray:
        ...
        for n in range(self.n_uav):
            ...
            if ds.size > 0:
                ...
                # Same task-independent demand proxy for explicit and hidden actors.
                # All 16 table modes are eligible proposals at every SNR.
                for k in ds:
                    if self.q_cache[n, k] > 0:
                        h_pend[n] += float(np.min(self.Lambda_sem[n, k, :]))
    ...
    def _build_action_available_masks(self) -> np.ndarray:
        ...
        for n in range(self.n_uav):
            ...
            # Do not expose the instruction via actor-side quality masks.
            # step() retains the common quality and budget feasibility checks.
            row[self.uav_act_slices["mu_logits"]] = 1.0
            masks[1 + n] = row
    ...
    def step(self, actions):
        ...
        wq, wa, wl = self.reward_weights_by_instruction[gid]
        quality_contrib = float(wq * quality_term)
        load_penalty = float(-wl * load_term)
        aoi_penalty = float(-wa * aoi_term)
        ...
        # concentrated in the instruction-dependent reward weights below.
        ...
        aoi_reduction = float(np.mean(np.maximum(A_t - A_next, 0.0)))
        recv_aoi_bonus = float(self.eta_recv_aoi_bonus * aoi_reduction / max(A_limit_g, 1.0e-9))
        auxiliary = -constraint_penalty_A_g * aoi_violation_max + recv_aoi_bonus
        common_evaluation_reward = base_reward + auxiliary
        reward = common_evaluation_reward if self.use_task_auxiliary_reward else base_reward
        reward_components = {
            ...
            "penalty_aoi_violation": float(-constraint_penalty_A_g * aoi_violation_max)
                if self.use_task_auxiliary_reward else 0.0,
            "bonus_aoi_reduction": float(recv_aoi_bonus) if self.use_task_auxiliary_reward else 0.0,
        }
