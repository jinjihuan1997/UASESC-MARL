...
class OnPolicyActorBuffer:
    ...
    def __init__(self, args, obs_space, act_space):
        ...
        hybrid_available_dim = getattr(act_space, "hybrid_available_dim", None)
        if act_space.__class__.__name__ == "Discrete":
            self.available_actions = np.ones(
                (self.episode_length + 1, self.n_rollout_threads, act_space.n),
                dtype=np.float32,
            )
        elif hybrid_available_dim is not None:
            self.available_actions = np.ones(
                (
                    self.episode_length + 1,
                    self.n_rollout_threads,
                    int(hybrid_available_dim),
                ),
                dtype=np.float32,
            )
        else:
    ...
    def after_update(self):
        """After an SC, copy the data at the last step to the first position of the buffer."""
