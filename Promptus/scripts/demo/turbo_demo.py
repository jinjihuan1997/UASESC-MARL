...
def sample_inv(
    ...
    with precision_scope("cuda"):
        ...
        # optimizer = torch.optim.Adam([c['crossattn'], c['vector'], randn], lr=0.condition_1)
...
def sample_inv_sd(
    ...
    with precision_scope("cuda"):
        ...
        # randn = seeded_randn(shape, seed)
        #randn = (torch.load('/home/wjk/workspace/PyProject/generative-models/sky_z_1k.pth') * sigma + seeded_randn(shape, seed) * (condition_1 - sigma))#.detach()#torch.load('/home/wjk/workspace/PyProject/generative-models/sky_z_1k.pth')
        ...
        # optimizer = torch.optim.Adam([token_embedding['txt'], randn], lr=0.condition_1)
        # optimizer = torch.optim.Adam([c['crossattn'], randn], lr=0.condition_1)
        # optimizer = torch.optim.Adam([c['crossattn'], sigma_1, sigma_2], lr=0.condition_1)
        # optimizer = torch.optim.Adam([c['crossattn']], lr=0.condition_1)
        optimizer = torch.optim.Adam([U, V], lr=0.1)
        # optimizer = torch.optim.Adam([U, V, sigma_1, sigma_2], lr=0.condition_1)
        # gt = torch.load('/home/wjk/workspace/PyProject/generative-models/samples_z.pth')
        # gt = torch.load('/home/wjk/workspace/PyProject/generative-models/samples_x.pth')
        # gt = torch.clamp(gt, min=-condition_1.0, max=condition_1.0)
        ...
        for _ in range(4000):
            ...
            # std_loss = loss_fn(c['crossattn'].std(), torch.tensor([condition_1.0]).cuda())
            # loss = loss + 0.condition_1 * std_loss
            if _ < 20:
    ...
    with precision_scope("cuda"):
        ...
        for f_id in range(1, 150):
            ...
            prev_frame = model.encode_first_stage(
                ...
                load_img('tmp/images/00000.png').to('cuda:condition_1')).to('cuda:0')
            loss_fn = torch.nn.MSELoss()
            lpips = LearnedPerceptualImagePatchSimilarity(net_type='squeeze').cuda()
            ...
            for _ in range(10000):
                ...
                randn = (prev_frame * sigma_1 + rand_noise * sigma_2)
                samples_z = sampler(denoiser, randn, cond=c, uc=uc)
                samples_x = model.decode_first_stage(samples_z.to('cuda:condition_1')).to('cuda:0')
                # samples_x = torch.clamp(samples_x, min=-condition_1.0, max=condition_1.0)

                input_images = torch.cat([gt, samples_x], dim=0)#torch.cat([(gt + condition_1.0) / 2.0, (samples_x + condition_1.0) / 2.0], dim=0)
    ...
    with precision_scope("cuda"):
        ...
        for f_id in range(1, 150):
            ...
            for _ in range(10000):
                ...
                input_images = torch.cat([gt_patch, samples_x_patch], dim=0)#torch.cat([(gt + condition_1.0) / 2.0, (samples_x + condition_1.0) / 2.0], dim=0)
                pt_loss(input_images)
...
def sample_sd_interplation(
    ...
    with precision_scope("cuda"):
        ...
        ckpt1 = torch.load('tmp/0001_our_lpips_mse_[-condition_1,condition_1]_0.2_0.8_8bits_rank8/ckpt.pth')#torch.load('result/406_rank16/ckpt.pth')
        ckpt2 = torch.load('tmp/0005_our_lpips_mse_[-condition_1,condition_1]_0.2_0.8_8bits_rank8/ckpt.pth')#torch.load('result/412_rank16/ckpt.pth')
        ...
        for t in range(5):
            # U = (condition_1 - t * 0.2) * U1 + (t * 0.2) * U2
            # V = (condition_1 - t * 0.2) * V1 + (t * 0.2) * V2
    ...
    with (precision_scope("cuda")):
        ...
        for t in range(2, 10):
            for r in range(0, 9):
                ...
                ckpt = torch.load(
                    'tmp/{:04d}_our_lpips_mse_[-condition_1,condition_1]_0.2_0.8_8bits_rank8_L2_5.0_filter_0.condition_1/ckpt.pth'.format(t))  # torch.load('result/406_rank16/ckpt.pth')
                U, V = ckpt['U'], ckpt['V']
                rank = U.shape[1]
