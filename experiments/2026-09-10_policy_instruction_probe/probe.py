...
def interventions(env,obs,visible):
    ...
    assert torch.equal(result[:,:,~allowed],
...
@torch.no_grad()
def forward_batch(actors,variants,available,env,validate_native=False):
...
def forward(actors,variants,available,env,validate_native=False):
    # Identical batch shape and row positions for each intervention. Concatenating
    # all variants can introduce GEMM roundoff for identical negative controls.
    rows=[forward_batch(actors,variants[i:i+1],available,env,validate_native) for i in range(len(variants))]
    return {key:np.concatenate([row[key] for row in rows],axis=1) for key in rows[0]}
...
def run_one(algorithm,seed,manifest):
    ...
    for scenario,schedule in manifest['scenarios'].items():
        ...
        store={k:[] for k in ['slot','seed','true_gid','available','observation_sha256','hidden_beta','hidden_prob',
            'cache_active','snr_db','mode_quality','mode_load','inference_batch_size']}
        ...
        for slot in range(600):
            ...
            if slot in points:
                ...
                hidden_variant=interventions(env,obs,False)
                pred_hidden=forward(hidden_actors,hidden_variant,available,env,validate_native=slot==0)
                batch_sizes=np.full(E,E,dtype=np.int16)
                # Original formal evaluation used batch size one. Near-tied mode
                # probabilities may change argmax with GEMM batch layout. For a
                # mismatching state use that original layout for EVERY variant.
                mismatch=np.any(pred['mode'][:,0]!=trace['mode'][:,slot],axis=1)
                for idx in np.flatnonzero(mismatch):
                    solo=forward(actors,variant[:,idx:idx+1],available[idx:idx+1],env)
                    solo_hidden=forward(hidden_actors,hidden_variant[:,idx:idx+1],available[idx:idx+1],env)
                    single_state_fallbacks.append(dict(slot=slot,seed=seeds[idx],
                        batch_modes=pred['mode'][idx,0].tolist(),single_modes=solo['mode'][0,0].tolist(),
                        historical_modes=trace['mode'][idx,slot].tolist()))
                    for key in pred:
                        pred[key][idx]=solo[key][0]
                        pred_hidden[key][idx]=solo_hidden[key][0]
                    batch_sizes[idx]=1
                ...
                store['cache_active'].append(array(env.q.any(-1)).copy())
                store['snr_db'].append(array(env.snr_db).copy())
                store['mode_quality'].append(array(env.quality).copy())
                store['mode_load'].append(array(env.load).copy())
                store['inference_batch_size'].append(batch_sizes)
        ...
        write(statusfile,dict(state='complete',scenario=scenario,probes=count,replay_slots=E*600,
            ...
            single_state_fallbacks=single_state_fallbacks,
