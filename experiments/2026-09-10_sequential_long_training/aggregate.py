...
def main(root=HERE,check_inputs=True):
    ...
    for j in m['jobs']:
        ...
        warmup=cfg['training_design']['resource_warmup_steps']
        for row in logs:
            fixed=j['method']=='staged' and (row['update']-1)*m['batch']<warmup
            assert row['trained_actor_ids']==([1,2,3] if fixed else [0,1,2,3])
            expected=1e-4*(1-(row['update']-1)/len(logs))
    ...
    for step in m['milestones']:
        ...
        paired[step]['staged_minus_joint']={scope:stats([per[f'seed_{s}/staged_at_{step}'][scope]['common_reward']-per[f'seed_{s}/joint_at_{step}'][scope]['common_reward'] for s in seeds]) for scope in scopes}
    ...
    lines=['# 长训练结果：联合与分阶段','',
        f'每个模型训练{m["steps_per_method"]}环境步，训练种子{seeds}。分阶段前{m["resource_warmup_steps"]}步固定均分资源。',
    ...
    lines+=['','本轮没有新增隐藏指令消融，不能单凭此结果判断显式指令输入的独立贡献。训练种子均值和正差数量不等同于统计显著性。']
