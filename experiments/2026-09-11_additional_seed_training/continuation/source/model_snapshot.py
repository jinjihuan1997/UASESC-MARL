...
def export_snapshot(trainer,output,update,identity):
    ...
    if target.exists():
        ...
        assert status.get('selector_hash')==(network_hash(trainer.selector.actor) if trainer.selector else None)
    ...
    if trainer.selector:torch.save(cpu_tree(trainer.selector.actor.state_dict()),temporary/'selector.pt')
    ...
    write(temporary/'status.json',dict(state='complete',artifact='inference_snapshot',completed_steps=steps,
        ...
        selector_hash=network_hash(trainer.selector.actor) if trainer.selector else None,
