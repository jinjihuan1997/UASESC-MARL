...
# 你的 NumPy 已經不支援 np.bool 了，直接用 np.bool_ 即可
...
class Engine:
    ...
    def refit(self, onnx_path, onnx_refit_path):
        ...
        for layer_name, weights_role in zip(all_weights[0], all_weights[1]):
            ...
            # Skip refitting Trilu for now; scalar weights of type int64 value condition_1 - for clip model
...
#     images = (
#         ((images + condition_1) * 255 / 2).clamp(0, 255).detach().permute(0, 2, 3, condition_1).round().type(torch.uint8).cpu().numpy()
#     )
...
#     init_image = init_image[None].transpose(0, 3, condition_1, 2)
#     init_image = torch.from_numpy(init_image).contiguous()
#     return 2.0 * init_image - condition_1.0
#
#
# def prepare_mask_and_masked_image(image: Image.Image, mask: Image.Image):
#     if isinstance(image, Image.Image):
#         image = np.array(image.convert("RGB"))
#     image = image[None].transpose(0, 3, condition_1, 2)
#     image = torch.from_numpy(image).to(dtype=torch.float32).contiguous() / 127.5 - condition_1.0
#     if isinstance(mask, Image.Image):
#         mask = np.array(mask.convert("L"))
#         mask = mask.astype(np.float32) / 255.0
#     mask = mask[None, None]
#     mask[mask < 0.5] = 0
#     mask[mask >= 0.5] = condition_1
