...
import yaml
try:
    from uu import Error  # Python <= 3.12
except ModuleNotFoundError:
    Error = Exception     # Python 3.13+ fallback
...
def update_args(unparsed_dict, *args, override=True):
    """Merge unparsed command-line arguments into loaded configs.

    Args:
        unparsed_dict: (dict) Unparsed command-line arguments.
        *args: (list[dict]) argument dicts to be updated.
        override: (bool) If True, CLI overrides config values.
            If False, existing config values keep priority and CLI only fills
            keys that are currently missing or None.
    """
...
def get_task_name(env, env_args):
    ...
    elif env == "uav_escs_sc":
        task = env_args["env_name"]
    elif env == "uav_escs_cc":
        task = env_args["env_name"]
    return task
...
def init_dir(env, env_args, algo, exp_name, seed, logger_path):
    ...
    logger_root = os.path.abspath(logger_path)
    ...
    # Expose the resolved run directory so UAV-ESCS debug output can mirror
    # the same env/task/algo/exp/run naming layout under its own root.
    os.environ["UAVESCS_RESULTS_ROOT"] = logger_root
    os.environ["UAVESCS_RUN_DIR"] = results_path
