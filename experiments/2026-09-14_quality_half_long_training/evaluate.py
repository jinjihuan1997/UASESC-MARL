...
def main():
 ...
 for step in ([x.steps] if x.steps else [2000000,4000000,6000000,8000000,10000000]):
  assert step in [2000000,4000000,6000000,8000000,10000000]
  for scene in (m['scenarios'] if step==10000000 else ['fixed_0','fixed_1','fixed_2']):rollout(x.seed,step,scene)
