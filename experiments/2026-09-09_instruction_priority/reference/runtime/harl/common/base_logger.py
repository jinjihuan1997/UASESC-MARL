...
class BaseLogger:
    ...
    def log_train(self, actor_train_infos, critic_train_info):
        ...
        def _scalar_or_none(x):
            try:
                # Support torch tensors (including requires_grad) without importing torch.
                if hasattr(x, "detach"):
                    x = x.detach()
                if hasattr(x, "cpu"):
                    x = x.cpu()
        ...
        for agent_id in range(self.num_agents):
            for k, v in actor_train_infos[agent_id].items():
                ...
                if sv is None:
                    print(
                        f"[logger] skip invalid train metric: {agent_k} value={v}",
                        flush=True,
                    )
                    continue
                self.writter.add_scalars(agent_k, {agent_k: sv}, self.total_num_steps)
        # log critic
        for k, v in critic_train_info.items():
            ...
            if sv is None:
                print(
                    f"[logger] skip invalid train metric: {critic_k} value={v}",
                    flush=True,
                )
                continue
            self.writter.add_scalars(critic_k, {critic_k: sv}, self.total_num_steps)
    ...
    def log_env(self, env_infos):
        ...
        for k, v in env_infos.items():
            if len(v) > 0:
                ...
                if mv is None:
                    print(
                        f"[logger] skip non-finite env metric: {k} value={v}",
                        flush=True,
                    )
                    continue
                self.writter.add_scalars(k, {k: mv}, self.total_num_steps)
