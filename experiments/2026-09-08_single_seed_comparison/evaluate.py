...
def external_metrics(env, info):
    ...
    aoi_term = (env.aoi_mean_weight*np.mean(after/env.A_max)
                + env.aoi_max_weight*np.max(after/env.A_max)
                + env.aoi_tail_weight*np.mean(np.maximum((after-env.aoi_tail_threshold)/env.A_max, 0)))
    base = env.omega_Q*q_gain - env.omega_Lambda*uses/env.Lambda_ref/env.n_uav - env.omega_A*aoi_term
