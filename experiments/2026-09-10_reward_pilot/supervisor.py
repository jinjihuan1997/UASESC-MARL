...
def training_progress(m):
    rows={};remaining={core:0. for core in range(6)}
    ...
    for item in m['jobs']:
        ...
        seconds=r.get('last_timing',{}).get('total_seconds',fallback)
...
def run_phase(m,phase,stopping,resume):
    items=m['jobs'] if phase=='training' else [dict(id=j['id'],output='evaluation/'+j['id']) for j in m['jobs']]+[dict(id=f'{o}/rules/{r}',output=f'evaluation/{o}/rules/{r}') for o in m['objectives'] for r in m['rules']]
