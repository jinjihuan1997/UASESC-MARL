...
def resolve_frame_path(path_str):
    path = Path(path_str)
    candidates = []
    if path.is_absolute():
        ...
        candidates.append((REPO_ROOT / 'experiment_scripts' / path).resolve())
...
class NFS_Video(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.block_size[-3]):
            img = Image.open(resolve_frame_path(vid[i]))
            im = F.to_tensor(img)
            ...
            video[i+8,:,:, :]=im
        for i in range(self.block_size[-3]):
            img = Image.open(resolve_frame_path(vid[i+8]))
            im = F.to_tensor(img)
            # im = im.permute(2, 0, 1)  # permute from numpy format to torch format
            # im = im / 255.0  # [0,1]
            im=self.trans(im)
            ...
            video[i,:,:, :]=im
        for i in range(self.block_size[-3]):
            img = Image.open(resolve_frame_path(vid[i+16]))
            im = F.to_tensor(img)
            # im = im.permute(2, 0, 1)  # permute from numpy format to torch format
            # im = im / 255.0  # [0,1]
            im=self.trans(im)
        ...
        for i in range(self.block_size[-3]):
            img = Image.open(resolve_frame_path(vid[i+24]))
            im = F.to_tensor(img)
            # im = im.permute(2, 0, 1)  # permute from numpy format to torch format
            # im = im / 255.0  # [0,1]
            im=self.trans(im)
