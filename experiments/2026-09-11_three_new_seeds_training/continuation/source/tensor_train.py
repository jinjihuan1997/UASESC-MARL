...
class TensorTrainer:
    ...
    def set_training_update(self, update):
        ...
        self.fixed_resources=(design['arm']=='mode_only' or
            (design['arm']=='staged' and (update-1)*self.batch<design['resource_warmup_steps']))
        self.trainable_agent_ids=list(range(1 if self.fixed_resources else 0,len(self.actors)))
        self.stage='fixed_equal_resources' if self.fixed_resources else 'joint'
    ...
    def collect(self):
        ...
        for t in range(b.length):
            ...
            if self.fixed_resources:
                action=torch.full((self.env.count,self.env.U),2/self.env.U-1,device=self.device)
