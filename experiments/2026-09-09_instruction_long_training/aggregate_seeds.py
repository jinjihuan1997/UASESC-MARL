...
def aggregate_run(run):
    ...
    if (run/'report').exists():
        ...
        return verification
    results, responses, reference_pairing = {}, {}, None
    ...
    for item in items:
        ...
        for scenario, schedule in manifest['scenarios'].items():
            ...
            for metric in ['mean_aoi','delivered_predicted_psnr','common_reward','deliveries_per_slot']:
                ...
                responses[item][scenario][metric] = dict(before_mean=statistics.mean(pre),
                    after_mean=statistics.mean(post), mean_change=statistics.mean(b-a for a,b in zip(pre,post)),
    ...
    for method in manifest['method_order']:
        ...
        if method != 'IC_HAPPO':
            differences = [results[f"seed_{seed}/IC_HAPPO"]['overall']['common_reward']-
    ...
    write(destination/'comparison.json', dict(seeds=manifest['seeds'], learned=learned, rules=rules,
        ...
        difference_direction='IC_HAPPO minus comparator; positive favors IC_HAPPO',
