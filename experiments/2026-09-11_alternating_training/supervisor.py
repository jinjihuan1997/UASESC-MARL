    ...
    try:
        ...
        while pending or active:
            ...
            if not thermal.cooling:
                ...
                for core in cores:
                    if len(active)>=m['resources']['maximum_active_workers']:break
            ...
            write(HERE/'status.json',dict(state='cooling' if thermal.cooling else 'training_and_evaluating',updated_utc=stamp(),supervisor_pid=os.getpid(),
                ...
                cpu_max_c=cpu,gpu_max_c=gpu,estimated_training_seconds_remaining=eta,eta_excludes_queueing=True,maximum_active_workers=m['resources']['maximum_active_workers'],failed_jobs=failures))
        ...
        write(HERE/'status.json',dict(state='complete',updated_utc=stamp(),completed_training_steps=m['total_training_steps'],completed_training_jobs=len(m['jobs']),
            evaluation_episodes=m['evaluation_episodes'],report=str(HERE/'REPORT.md'),science_status='paired_short_training_and_new_validation_complete_final_test_unused'))
        return 0
