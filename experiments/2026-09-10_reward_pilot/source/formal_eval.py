...
def evaluate_one(run, item_id):
    ...
    if learned:
        ...
        # Rules use the complete system, irrespective of queue/device order.
        full_system_job = next(j for j in manifest['jobs'] if j['method'] == 'IC_HAPPO')
        config = read(run/full_system_job['config'])
    ...
    if (output/'config.json').exists() and read(output/'config.json') != args:
        raise ValueError('Evaluation environment configuration changed')
