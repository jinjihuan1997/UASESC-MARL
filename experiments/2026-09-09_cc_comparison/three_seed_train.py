"""Prepare, run, resume and inspect six CC jobs alongside the frozen SC queue."""
import argparse
...
def run_all(run, resume=False, allow_reference_active=False, train_only=False):
    ...
    sc_manifest=read(Path(manifest['parent']['parent_run'])/'manifest.json')
    sc_cores=set(sc_manifest['resources']['cpu_cores']+sc_manifest['resources']['gpu_host_cores']+sc_manifest['resources']['evaluation_cpu_cores'])
    if sc_cores.intersection(manifest['resources']['cpu_cores']+manifest['resources']['evaluation_cpu_cores']):
        raise RuntimeError('CC and frozen SC CPU allocations overlap')
    ...
    try:
        ...
        if train_only:
            write(run/'status.json', dict(state='training_complete_evaluation_pending', updated_utc=stamp(),
                seeds=manifest['seeds'], completed_training_jobs=len(manifest['jobs']), completed_training_steps=manifest['total_training_steps']))
        else:
            subprocess.run(['taskset', '-c', str(manifest['resources']['evaluation_cpu_cores'][0]), sys.executable, str(run/'source/aggregate_seeds.py'),
                '--run', str(run)], check=True)
            verification = read(run/'report/verification.json')
            if not manifest['purpose'].startswith('smoke'):
                subprocess.run(['taskset','-c',str(manifest['resources']['evaluation_cpu_cores'][0]),sys.executable,
                    str(run/'source/compare_sc_cc.py'),'--cc-run',str(run)],check=True)
            write(run/'status.json', dict(state='complete', updated_utc=stamp(), seeds=manifest['seeds'],
                completed_training_jobs=len(manifest['jobs']), completed_training_steps=manifest['total_training_steps'], report=verification))
        return 0
