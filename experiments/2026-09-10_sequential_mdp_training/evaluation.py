...
def evaluate_item(item,stopping):
    ...
    cfg=config(method if learned else 'joint',int(category[5:]) if learned else 85)
    actors=None
    if learned:
    ...
    for scenario,schedule in manifest['scenarios'].items():
        ...
        for slot in range(600):
            predicted=None
            if learned:actions=learned_actions(env,actors,obs,masks,method)
            ...
            elif method=='R_equal_single':actions=rule_actions(env,selection['equal']['single'])
            elif method=='R_equal_instruction':actions=rule_actions(env,selection['equal']['by_instruction'][int(env.context()[0][0])])
            elif method=='R_equal_myopic':actions,predicted=myopic_actions(env,equal=True)
