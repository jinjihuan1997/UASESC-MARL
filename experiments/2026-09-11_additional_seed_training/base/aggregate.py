...
def main():
    m=verify();assert len(m['jobs'])==1
    job=HERE/m['jobs'][0]['output'];s=read(job/'status.json')
    ...
    write(HERE/'report/audit.json',dict(state='PASS',episodes=0,training_steps=10000000,
        final_model_and_snapshot_verified=True,learning_rate_and_budget_verified=True,evaluation='Deferred to continuation phase'))
    (HERE/'REPORT.md').write_text('新种子joint基础模型完成1000万步；最终模型与快照已核验。下一阶段自动继承同一检查点，训练选择器和原方案续训对照。\n')
