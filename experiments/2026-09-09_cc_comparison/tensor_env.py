"""CC average-profile environment using the frozen SC state and reward equations.
...
class TensorCCEnv:
    def __init__(self, args, count=10, seed=1, device='cuda:0'):
        ...
        if not p.semantic_library.profile or p.semantic_mode_selection != 'all_modes':
            raise ValueError('CC requires an explicit average table and all_modes')
        ...
        with np.load(args['cc_delivery_moments_path'],allow_pickle=False) as moments:
            if p.Q_min_eval!=float(moments['q_min_eval']) or p.Q_max!=float(moments['q_normalization_reference']):
                raise ValueError('CC joint reward moments do not match the common SC objective')
            np.testing.assert_array_equal(moments['snr_grid_db'],profile['snr_grid_db'])
            np.testing.assert_allclose(moments['success_psnr_mass']+moments['failure_psnr_mass'],profile['q_hat_mean'],atol=1e-12,rtol=0)
            self.profiles=torch.cat([self.profiles,self.tensor(np.stack([moments[k] for k in
                ('deliver_prob','success_psnr_mass','failure_psnr_mass','success_gain_mass')]))])
        # H264 headers, LDPC padding and modulation are already in the table.
        p.side_info_bits=0.
    ...
    def reset(self):
        ...
        # Independent channel-success stream: never consumes topology/content/task RNG.
        uniforms=[np.random.default_rng(np.random.SeedSequence([env._base_seed,e['episode_index'],20260909,771]))
                  .random((env.max_steps,self.U,self.K)) for env,e in zip(self.source.envs,episodes)]
        self.delivery_uniforms=self.tensor(np.stack(uniforms,axis=1))
    ...
    def update_tables(self):
        ...
        self.quality,self.lz,self.nz,self.delivery_probability,good_mass,bad_mass,gain_mass=value.unbind(0)
        self.delivery_probability=self.delivery_probability.clamp(0,1)
        self.success_quality=torch.where(self.delivery_probability>0,good_mass/self.delivery_probability.clamp_min(1e-300),0.)
        self.failure_quality=torch.where(self.delivery_probability<1,bad_mass/(1-self.delivery_probability).clamp_min(1e-300),0.)
        self.success_quality_gain=torch.where(self.delivery_probability>0,gain_mass/self.delivery_probability.clamp_min(1e-300),0.)
        self.load=self.lz
    ...
    def step(self, actions, auto_reset=True):
        ...
        attempted=torch.zeros_like(self.q).scatter(-1,ranks,ordered)
        attempt_count=attempted.sum(-1)
        probability=self.delivery_probability.gather(-1,mode[:,:,None]).squeeze(-1)
        delivered_quality=self.success_quality.gather(-1,mode[:,:,None]).squeeze(-1)
        delivered_gain=self.success_quality_gain.gather(-1,mode[:,:,None]).squeeze(-1)
        served=attempted & (self.delivery_uniforms[self.step_index]<probability[:,:,None])
        count=served.sum(-1)
        mode=torch.where(attempt_count>0,mode,-1)
        ...
        admission=~self.q
        # Single attempt, no ARQ: consume the transmitted block even if decoding fails.
        self.tau=torch.where(admission,self.step_index,torch.where(self.q & ~attempted,self.tau,-1))
        self.q=(self.q & ~attempted)|admission
        qgain=delivered_gain*count
        usage=chosen_load*attempt_count
        quality_term=qgain.sum(-1)/self.D
        load_term=(usage/p.Lambda_ref).sum(-1)/U
        norm=next_aoi/p.aoi_reward_ref
        aoi_term=p.aoi_mean_weight*norm.mean((-1,-2))+p.aoi_max_weight*norm.amax((-1,-2))+p.aoi_tail_weight*((next_aoi-p.aoi_tail_threshold)/p.aoi_reward_ref).clamp_min(0).mean((-1,-2))
        weights=self.reward_weights[gid]
        base=weights[:,0]*quality_term-weights[:,2]*load_term-weights[:,1]*aoi_term
        ...
        self.last=dict(mode=mode,served=served,attempted=attempted,failed=attempted & ~served,
            admission=admission,usage=usage,quality=delivered_quality,expected_quality=chosen_quality,
            selected_delivery_probability=probability,quality_gain=qgain,
            old_aoi=old_aoi,next_aoi=next_aoi,cache_before=old_q,tau_before=old_tau,
